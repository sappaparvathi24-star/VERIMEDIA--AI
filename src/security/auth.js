  ...
  if (rawApiKey) {
    ...
    if (ALLOWED_API_KEY_HASHES.has(hashed)) {
      ...
      try {
        ...
        if (row) {
          const userObj = { id: row.id, email: row.email, role: row.role, organizationId: row.organization_id };
      ...
      // Env-configured key with no DB row: grant service-account ANALYST role
      req.user = {
